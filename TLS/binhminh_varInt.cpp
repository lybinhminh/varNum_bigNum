...
// normalize the current varInt and return reference to it
...
varInt varInt::normalize() {
	...
	for (int i = length-1; i > 0; --i) {
		unsigned char b = data[i], nB = data[i - 1];
		if (neg && b == 0xFF && (nB & 0x80) == 0x80)j--;
		else if (!neg && b == 0x00 && (nB & 0x80) == 0x00)j--;
		else break;
	}
	if (j < length)
	{
...
varInt varInt::pow(varInt a, varInt b) {
	if (b < varIntConst::ZERO)return varInt(0);
	varInt power(1); varInt exponent(b.data, b.length, b.length);
	while (exponent > varIntConst::ZERO) {
		power *= a;
		exponent-= varIntConst::ONE;
	}
	exponent.discard();
	return power;
}
...
varInt varInt::barretReduction(varInt x, varInt& n) {
	...
	size_t k = n.length * 8 - countLeadingBit(n, 0);
	varInt m;
	if (!n._barret_cache_)
	{
		varInt tpk(2); tpk <<= k - 1; m = tpk.naiveDivi(n, NULL);
		n.barret_cache = new varInt(m.data,m.length,m.length);
		n._barret_cache_ = true;
		tpk.discard();
		m.discard(); m = *n.barret_cache;
	}
	else{
		m = *n.barret_cache;
	}
...
varInt varInt::fromString(const char* str) {
	bool hexadecimal = false, neg = false;
	int i = strlen(str)-1, j = 0, a = 0;
	if (str[0] == '-') {
		neg = true;
		a = 1;
	}
	if (str[a] == 48 && str[a + 1] == 'x') { 
		hexadecimal = true; 
		j = a + 2;
	}
	varInt base(1);
	...
	for (; i >= j; --i) {
		int num = str[i] - 48;
		if (num > 9 && hexadecimal) num -= 7;
		if (num > 15) num -= 32;
		varInt v(num);
		v *= base; 
		base *= hexadecimal ? varIntConst::HEX : varIntConst::TEN;
		result += v;
		v.discard();
	}
	base.discard();
...
char* varInt::toString(varInt a) {
	...
	while (copy > varIntConst::ZERO) {
		varInt remainder, c = copy.naiveDivi(varIntConst::TEN, &remainder);
		free(copy.data);
		copy.data = c.data;
...
varInt varInt::gcd(varInt a, varInt b) {
	if (a == varIntConst::ZERO) {
		return varInt(b.data, b.length, b.length);
	}
	if (b == varIntConst::ZERO) {
		return varInt(a.data, a.length, a.length);
	}
	varInt d = abs(a), r0 = abs(b);
	if (a < b)
		swap(d, r0);
	while (r0 > varIntConst::ZERO) {
		varInt r1, q = d.naiveDivi(r0, &r1); q.discard();
...
varInt varInt::bezoutIdentity(varInt a, varInt b, varInt* x, varInt* y) {
	...
	while (r0 > varIntConst::ZERO) {
		varInt r1, q = d.naiveDivi(r0, &r1);
		varInt qs1 = q * s1, qt1 = q * t1; q.discard();
		varInt s2 = s0 - qs1, t2 = t0 - qt1; qs1.discard(); qt1.discard();
		s0 = s1; t0 = t1;
		s1 = s2; t1 = t2;
		d.discard(); d = r0;
		r0 = r1;
...
varInt varInt::barretDivision(varInt* n,varInt* remainderOut) {
	varInt aq;
	size_t k = n->length * 8 - countLeadingBit(*n, 0);
	if (!n->_barret_cache_) {
		...
		aq = 1; aq <<= k;
		aq /= *n;
		varInt *aqc = new varInt(aq.data, aq.length, aq.length);
		n->barret_cache = aqc;
	...
	varInt aqn = aq * *n; aqn >> k;
	varInt r = *this - aqn;
	varInt error(0);
	while (!(r < *n)) {
		r -= *n; r.normalize();
		error += varIntConst::ONE;
	}
...
varInt& varInt::negate() {
	for (int i = 0; i < length; ++i) {
		data[i] = ~data[i];
	}
	*this += varIntConst::ONE;
	return *this;
}
varInt& varInt::abs() {
	if (isNegative(*this))
...
size_t varInt::log10(varInt v, varInt* d) {
	varInt base(10);
	size_t i = 1;

	for (; !(v < base) ; ++i) {
		base *= varIntConst::TEN; base.normalize();
	}
	i--;
	base /= varIntConst::TEN;
	if (d != NULL) {
		d->discard();
		*d = base;
	}
...
size_t varInt::log2(varInt v, varInt* d) {
	size_t i = v.length * 8 - countLeadingBit(v, 0);
	i = i < 1 ? 0 : i - 1;
	if (d != NULL) {
		d->discard();
		*d = 1;
